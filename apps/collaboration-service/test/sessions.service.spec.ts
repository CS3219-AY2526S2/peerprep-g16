...
describe('SessionsService', () => {
  ...
  describe('updateWhiteboard', () => {
    it('updates whiteboard elements', async () => {
      ...
      expect(service.findOne('match-123')?.whiteboardElements).toEqual(
        elements,
      );
    });
  ...
  describe('endSession', () => {
    ...
    it('publishes session.completed event to Redis stream', async () => {
      ...
      expect(redisMock.xadd).toHaveBeenCalledWith(
        ...
        expect.anything(),
        expect.anything(), // userAId
        expect.anything(),
        expect.anything(), // userBId
        expect.anything(),
        expect.anything(), // questionId
        expect.anything(),
        expect.anything(), // questionTitle
        expect.anything(),
        expect.anything(), // topic
        expect.anything(),
        expect.anything(), // difficulty
        'code',
        'print(1)',
        'language',
        'python',
        expect.anything(),
        expect.anything(), // hintsUsed
        expect.anything(),
        expect.anything(), // testCasesPassed
        expect.anything(),
        expect.anything(), // duration
        expect.anything(),
        expect.anything(), // whiteboardScreenshot
      );
    });
