...
export class MatchService {
  ...
  async findMatch(
    ...
    if (isRandom) {
      ...
      match = allUserData.find((u) => u.difficulty === difficulty);
      ...
      match = allUserData.find(
        (u) =>
          (u.topic === topic || u.topic === 'Random') &&
          canMatch(
            currentUserData.originalDifficulty,
            u.originalDifficulty,
            difficulty,
      ...
      match = allUserData.find(
        (u) => u.topic === topic && u.difficulty === difficulty,
      );
      ...
      if (!match)
        match = allUserData.find(
          (u) => u.topic === 'Random' && u.difficulty === difficulty,
        );
      // Priority 4: fully random user
      if (!match)
        match = allUserData.find(
          (u) => u.topic === 'Random' && u.difficulty === 'Random',
        );
    }
  ...
  async getQueueStatus(userId: string) {
    ...
    if (elapsed >= STAGE1_TIMEOUT) {
      ...
      await this.client.hset(`user:${userId}`, ['difficulty', 'Random']);
  ...
  async peekQueueStatus(userId: string) {
    ...
    return {
      status:
        elapsed >= STAGE1_TIMEOUT ? 'expand_search_difficulty' : 'waiting',
      message: 'Searching for a match...',
      elapsed,
      preferences: { topic: userData.topic, difficulty: userData.difficulty },
    };
...
function canMatch(
  originalSearchDiff: string,
  originalTargetDiff: string,
  searchDiff: string,
  targetDiff: string,
): boolean {
  if (originalSearchDiff === 'Random' || originalTargetDiff === 'Random')
    return true;
  ...
  if (searchDiff === 'Random' && targetDiff !== 'Random')
    return originalTargetRank <= originalSearchRank;
  if (targetDiff === 'Random' && searchDiff !== 'Random')
    return originalSearchRank <= originalTargetRank;
