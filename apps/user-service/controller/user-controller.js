  ...
  try {
    ...
    if (isAdmin !== undefined) {
      ...
      if (req.user.id === userId && isAdmin === false) {
        return res
          .status(403)
          .json({ message: 'Admins cannot remove their own admin privilege' });
      }
...
export async function getUserHistory(req, res) {
  try {
    ...
    const questionIds = await AttemptModel.find({ userId }).distinct(
      'questionId',
    );
    return res
      .status(200)
      .json(questionIds.map((qid) => ({ questionId: qid })));
  } catch (err) {
    console.error(err);
    return res
      .status(500)
...
export async function getUserAttempts(req, res) {
  try {
    ...
    const attempts = await AttemptModel.find({ userId }).sort({
      attemptedAt: -1,
    });
    return res
      .status(200)
      .json({ message: `Found attempts for user ${userId}`, data: attempts });
  } catch (err) {
    console.error(err);
    return res
      .status(500)
