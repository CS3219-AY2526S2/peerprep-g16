  ...
  redisSubscriber.on("message", async (channel, message) => {
    if (channel === ROLE_CHANGE_CHANNEL) {
      try {
        ...
        console.log("Privilege change event received for user: " + event.userId);
...
async function blacklistUser(userId, timestamp) {
  ...
  console.log("Blacklisted tokens for user " + userId + " issued before " + new Date(timestamp * 1000).toISOString());
}
...
export async function publishPrivilegeChange(userId, oldIsAdmin, newIsAdmin) {
  ...
  console.log("Published privilege change: user " + userId + " isAdmin " + oldIsAdmin + " -> " + newIsAdmin);
}
